...
try:
    import pyfftw as fft  #need to install this pip package
...
# Otherwise, fall back to 2 ** n padded scipy FFTPACK
    ...
    _FFT_NEXT_POW2 = True
    #_FFT_NEXT_POW2 = False

    def fft_kwargs(signal, **kwargs):
        """Return next higher power of 2 for given signal to speed up FFT"""
        #get the parameter n for fft to define padding, if n=len(signal) no padding with 0
        if _FFT_NEXT_POW2:
...
def ar1_spectrum(freqs, ar1=0.):
    ...
    freqs : numpy.ndarray, list
        Normalized frequency at which to calculate the theoretical power spectrum. array from [0-1/2]
    ar1 : float
    ...
    #the same as above
    Pk = (1 - ar1 ** 2) / (1 + ar1 ** 2 - 2 * ar1 * np.cos(2 * np.pi * freqs))
