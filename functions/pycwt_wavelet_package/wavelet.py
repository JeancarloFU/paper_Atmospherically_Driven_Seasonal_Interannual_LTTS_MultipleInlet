...
from helpers import (ar1, ar1_spectrum, fft, fft_kwargs, find, get_cache_dir,rednoise) #fft is from pyfftw (for parallel) or classical scipy.fftpack
...
def cwt(signal, dt, dj=1/12, s0=-1, J=-1, wavelet='morlet', freqs=None):
    ...
    """
    wavelet = check_parameter_wavelet(wavelet)
    ...
    if freqs is None:
        ...
        # Number of scales
        if J == -1:
            #as of Torrence and Compo (1998), eq. (10)
            J = int(np.round(np.log2(n0 * dt / s0) / dj))
        # The scales as of Mallat (1999) or as of Torrence and Compo (1998) eq. (9)
        sj = s0 * 2 ** (np.arange(0, J + 1) * dj)
        # Fourier equivalent frequencies. Table 1 of Torrence and Compo (1998)
        # for the Morlet, fourier_period = wavelet.flambda()*scales = 1.03*scales,
    ...
    # by default in helpers it includes padding, so this fft which is input for wavelets contains padding
    signal_ft = fft.fft(signal, **fft_kwargs(signal))
    N = len(signal_ft)
    #print("---------N with 0 padding = ", N)
    # Fourier angular frequencies: 2*pi*k/(N*dt) for k=[0-N/2] and -2pi*(N-k)/(Ndt) for k=[N/2-N[
    # = eq (5) of Torrence and Compo (1998), but with corrections to have symetric freqs.
    #So we have first the half (+) freqs and then the symmetric (-) freqs
    #starting from: 2pi*k/(Ndt), then we could evaluate at k-N (which doesnt affect the harmonics),
    #so finally the negatives freq should be: -2pi*(N-k)/(Ndt) for k=[N/2-N[ which is consistent with symmetric negative ftfreqs
    ...
    # (ii)  Calculate 2D matrix [scales, ftfreqs] for each scale s and Fourier angular frequency f (sj_col[a,1] * ftfreqs[b] = arr[a,b])
    # (iii) Calculate wavelet transform;
    #
    # eq (5, 6) and Table1 and fig2a-right of Torrence and Compo (1998)
    # fourier transform of wavelet function
    sj_col = sj[:, np.newaxis]
    cte = (sj_col * ftfreqs[1] * N)**.5 # ftfreqs[1]*N = 2*pi/(N*dt) * N = 2*pi/dt [as Torrence and Compo (1998)]
    #sw=sj_col * ftfreqs #sw[scale,ftfreqs]
    psi_ft_bar = cte * np.conjugate(wavelet.psi_ft(sj_col * ftfreqs)) #here we also have (-) ftfreqs. For (-) values wavelet.psi_ft is 0 (Table1 and fig2a-right)
    # eq (4) of Torrence and Compo (1998)
    W = fft.ifft(signal_ft * psi_ft_bar, axis=1, **fft_kwargs(signal_ft, overwrite_x=True)) #inverse in ftfreqs (axis=1) so get W[scales,time]
    #
    #check eq (7)  of Torrence and Compo (1998)
    #sum of fourier transform of wavelet function in angular ftfreqs for each scale should be = n (len of original data or padding one)
    #and check the % of cases that are least close to 10 units from value "n"
    #nwave=np.sum(psi_ft_bar**2,axis=1) #only values far from boundaries are =n, the others are less.
    #np.sum(nwave>(N-10))/len(nwave)*100 #around 90%

    # Checks for NaN in transform results and removes them from the scales if
    # needed, frequencies and wavelet transform.
    sel = np.invert(np.isnan(W).all(axis=1)) #for each scale is True only if all the W at this scale are nan
    #sel = ~(np.isnan(W).all(axis=1)) #same as above
    if np.any(sel): sj = sj[sel]; freqs = freqs[sel]; W = W[sel,:]

    # Determines the cone-of-influence. Note that it is returned as a function
    # of time in Fourier periods. Uses triangualr Bartlett window with non-zero end-points.
    coi = (n0 / 2 - np.abs(np.arange(0, n0) - (n0 - 1) / 2))
    cte = wavelet.flambda() * wavelet.coi() * dt
    coi = cte * coi

    #here we are selecting for wavelet the original n0 time values (excluding padding)
    #so trims wavelet transform at length n0.
    #only half power for classical fourier transform of the signal
    return (W[:, :n0], sj, freqs, coi) #, signal_ft[1:N//2+1] / N ** 0.5, np.abs(ftfreqs[1:N//2+1]) / (2 * np.pi)) #abs to remove - sign for nyquist freq
########################################################
...
def icwt(W, sj, dt, dj=1/12, wavelet='morlet'):
    ...
    cte= np.abs( dj * np.sqrt(dt) / (wavelet.cdelta * wavelet.psi(0)) )
    iW = cte * (np.real(W) / np.sqrt(sj)).sum(axis=0)
    return iW
########################################################
...
def significance(signal, dt, scales, sigma_test=0, alpha=None,
    ...
    -------
    signif : array like
        Significance levels as a function of scale or period (and scaled with variance of signal, =1 for standarized data)
    fft_theor (array like):
        Theoretical red-noise spectrum as a function of scale or period.
    ...
    # equation 16.
    #
    #this is the original function from the package pycwt (which is not ok)---
    #it only works if signal = float with variance of input data, so n0=N=1
    #def pk_original(k, a, N):
    #    return (1 - a ** 2) / (1 + a ** 2 - 2 * a * np.cos(2 * np.pi * k / N))
    #fft_theor = pk(freq, alpha, n0)
    #
    #our fixing to the above function---
    #
    #method 1)
    #def pk_new(k, a):
        #return (1 - a ** 2) / (1 + a ** 2 - 2 * a * np.cos(2 * np.pi * k)) 
        #return (1 - a ** 2) / (np.abs(1 - a * np.exp(-2 * np.pi * 1j * k)))**2 #the same as above
    #fft_theor = pk(freq, alpha) #freq=Normalized frequency
    #
    #or method 2) = the same results as 1)
    fft_theor = ar1_spectrum(freq, alpha)
    ...
    elif sigma_test == 1:  # get significance for Time-averaged power (global wavelet)
        if len(dof) == 1:
            #dof = np.zeros(1, J+1) + dof
            dof = np.zeros((1, int(J)+1)) + dof
        sel = find(dof < 1)
        dof[sel] = 1
    ...
    elif sigma_test == 2:  # get significance for scale-averaged power
        if len(dof) != 2:
...
############# Below functions were not used in the current paper ######################
...
def xwt(y1, y2, dt, dj=1/12, s0=-1, J=-1, significance_level=0.95,
    ...
    xwt (array like):
        ...
        wavelet.
    axwt : phase angle of Cross wavelet (the delay between y1 and y2 at time t on a scale s)
    coi (array like):
    ...
    W12 = W1 * W2.conj()
    # calculate the angle of W12 complex number
    axwt = np.angle(W12)
    ...
    if normalize:
        std1 = std2 = 1.
    #a1, _, _ = ar1(y1);   a2, _, _ = ar1(y2)
    #in case above fail use below
    a1=np.corrcoef(y1[:-1],y1[1:])[0,1]; a2=np.corrcoef(y2[:-1],y2[1:])[0,1]
    #
    Pk1 = ar1_spectrum(freq * dt, a1);   Pk2 = ar1_spectrum(freq * dt, a2)
    dof = wavelet.dofmin
    PPF = chi2.ppf(significance_level, dof)
    signif = (std1 * std2 * (Pk1 * Pk2) ** 0.5 * PPF / dof)

    # The resuts:
    return W12, axwt, coi, freq, signif
########################################################
...
def wct_significance(al1, al2, dt, dj, s0, J, significance_level=0.95,
    ...
    if cache:
        ...
        #
        #original (problems with name for lags in files)
        ...
        #new
        cache_file = 'wct_sig_{:0.5f}_{:0.5f}_{:0.5f}_{:0.5f}_{:d}_{}'\
            .format(al1, al2, dj, s0 / dt, J, wavelet.name)
...
########################################################
def check_parameter_wavelet(wavelet):
